...
Last week we looked at some advanced topics: Sigma protocols and the OMDL and AMODL assumptions. In this week, we will review and reinforce our understanding of the Schnorr security proof, as well as practice writing security definitions for new protocols. We will also briefly introduce Schnorr multi-signatures.\\

Before we begin, let us take a moment to take a pass at range proofs.

\begin{exercise} Recall that for a group $\G$ of order $p$ with generators  $g$ and $h$, a Pedersen commitment to $x\in\Z_p$ with randomness $y\in\Z_p$ is of the form $C = g^xh^y$.
\begin{enumerate}[(a)]
